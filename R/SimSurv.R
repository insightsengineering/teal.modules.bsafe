...
sim1Study <- function(nPat = c(g1 = 100, g2 = 100),
  ...
  res <- matrix(data = NA, nrow = N, ncol = 7,
                dimnames = list(ID = c(1:N),
                                c("gID", "ID", "Entry", "EventTime", "ObsTime",
...
simTestData <- function(nStud = 5, tau = 0.05,
  ...
  res <- array(data = NA, dim = c(nSets, nStud+1, 5 ),
               dimnames = list(set = c(paste0("set",1:nSets)),
  ...
  for(i in 1:nSets){
    ...
    for(h in 1:nStud){
      ...
      tempStudy <-  sim1Study(nPat = setArray[i, ,"nPat"],
                        hz = tempHz,
                        dropout = dropout,
                        accr = accr,
                        nObsEvt = nObsEvt)
