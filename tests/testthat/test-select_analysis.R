local({
  ...
  test_that("get_safety_topic_choices_and_selection returns correct choices when there are choices", {
    choices_selected <- get_sfty_choices_select(data, "", c("A", "B"))
  ...
  test_that("get_safety_topic_choices_and_selection returns correct choices when previous selection is not empty", {
    choices_selected <- get_sfty_choices_select(data, "Z", c("A", "B"))
    expect_equal(choices_selected[[1]], c("X", "Y", "Z"))
  })
  ...
  test_that("get_safety_topic_choices_and_selection returns NULL choices when there are no choices", {
    choices_selected <- get_sfty_choices_select(data, "", c("D"))
...
local({
  ...
  app <- shinytest2::AppDriver$new(teal.modules.bsafe:::mock_select_analysis_mod())
  wait_value_idle(app, input = tns(BSAFE_ID$SEL_TRT))
  ...
  test_that("exported data code matches output", {
    expected_val <- shiny::isolate(app$get_values()[["export"]][["r"]][["data"]]())
    current_val <- rlang::eval_tidy(shinymeta::expandChain(shiny::isolate(app$get_values()[["export"]][["r"]][["data"]]())))
