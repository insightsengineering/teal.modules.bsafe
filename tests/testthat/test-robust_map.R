local({
  test_that("preface_rob_txt returns correct preface for weakly informative conjugate component", {
    ...
    rob_weight <- 0.5
    rob_mean <- 0.25
    expected <- "Based on a weakly informative conjugate component with weight w = 0.5 and mean 0.25 the robust MAP prior is approximated as (1-w) * (MAP Prior) + w * (weakly informative prior): "
  ...
  test_that("preface_rob_txt returns correct preface for EX-NEX approach", {
    ...
    rob_weight <- 0.75
    rob_mean <- 0.35
    expected <- "Based on a weakly informative conjugate component with w =  0.75 and mean 0.35 on the exp scale (-1.0498 on the log scale) of the robust MAP prior is approximated as  (1 - w) * (MAP prior) + w * (weakly informative prior): "
    result <- preface_rob_txt(sel_analysis, rob_weight, rob_mean)
    expect_equal(result, expected)
  })
